...
! Grid parameters.
    ...
    pgrid%nlong     = nlon
    pgrid%dr0       = ddepth
    pgrid%dlat0     = dlat * deg_to_rad
    pgrid%dlong0    = dlon * deg_to_rad
    pgrid%lat0      = lat0 * deg_to_rad
    pgrid%long0     = lon0 * deg_to_rad
...
! We have a pp type phase.
          npp_receiver(irx) = npp_receiver(irx) + 1
          PRINT *, "WE HAVE A pP TYPE PHASE", npp_receiver(irx)
        ENDIF
      ENDDO
    ENDDO
...
! point to it.
          IF (rx%source_equivalent == 0) THEN
            ...
            rx%source_equivalent = n_sources_ppinc
